...
class PandaSim(object):
  def __init__(self, bullet_client, offset):
    ...
    self.plainId = self.bullet_client.loadURDF("plane.urdf", [0+offset[0], 0+offset[1], -0.61+offset[2]], flags=flags)
    ...
    # 设置 Panda 机械臂初始姿态
    orn=[-0.707107, 0.0, 0.0, 0.707107]  # 四元数，等价于欧拉角[-pi/2, pi/2, 0]
  ...
  def step(self):
    # 根据状态控制夹爪目标开合度
    if self.state==6:
      self.finger_target = 0.01  # 抓取
    if self.state==5:
      self.finger_target = 0.04  # 松开
